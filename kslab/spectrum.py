...
class Spectrometer(object):
    ...
    def __init__(self,path,exception_word=["test","background","Background"]):
        ...
        df = [
            [data_list[n], min(data_list[n][1]), data_list[n][1][np.argmax(data_list[n][0])],max(data_list[n][1]),get_rot(i),i,join(path,i)] for n,i in enumerate(ls)
            ]

        self.columns = [
            "data","wavelenght(min)","wavelength(spectrum)", "wavelenght(max)","rotation","title","path"
            ]
        self.df = pd.DataFrame(data=df,columns=self.columns)

        self.df["name"] = self.df["wavelength(spectrum)"].map(self.spec_name)
        self.df["O2 %"] = self.df["title"].map(O2_per)
        self.df = self.df.sort_values(by="rotation").reset_index(drop=True)
        self.df = self.df[
            ["data","name","O2 %","wavelenght(min)","wavelength(spectrum)", "wavelenght(max)","rotation","title","path"]
            ]
        self.path = path
    ...
    def gauss(self,num_gauss=[1,2,2,1,1],single=False,disp=True):
        ...
        df = pd.DataFrame(
            columns=["data","name","O2 %","wavelenght(min)","wavelength(spectrum)", "wavelenght(max)","rotation","title","path"]
            )
        ...
        for i,num in zip(df_name,num_gauss):
            ...
            for n,j in enumerate(i["data"]):
                ...
                lim = [j[1][np.argmax(j[0])] - 0.16,j[1][np.argmax(j[0])] + 0.16] 
    ...
    def temperature(self):
        ...
        for i in range(len(self.df_gauss)):
            self.df_gauss["temperature1"][i] = calc_temp((self.df_gauss["fwhm1"][i]**2 - self.df_gauss["fwhm_function"][i]**2)**0.5,self.df_gauss["wavelength(spectrum)"][i],tar=self.df_gauss["name"][i])
            self.df_gauss["temperature2"][i] = calc_temp((self.df_gauss["fwhm2"][i]**2 - self.df_gauss["fwhm_function"][i]**2)**0.5,self.df_gauss["wavelength(spectrum)"][i],tar=self.df_gauss["name"][i])
...
def O2_per(path):
    ...
    return "0%" if per==[] else per[0].split(".")[0]
...
def calc_temp(fwhm,wavelength,tar = "H"):

    A=15.999 if tar=="OI" else  1.0079
