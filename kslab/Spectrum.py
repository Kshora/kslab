...
class Spectrometer(object):
    def __init__(self,path,exception_word=["test","background","Background"]):
        ...
        self.df = self.df[["data","name","O2 %","wavelenght(min)","wavelength(spectrum)", "wavelenght(max)","rotation","title","path"]]
        self.path = path
    ...
    def gauss(self,num_gauss=[1,2,2,1,1],single=False,disp=True):
        ...
        df = pd.DataFrame(columns=["data","name","O2 %","wavelenght(min)","wavelength(spectrum)", "wavelenght(max)","rotation","title","path"])
...
def display_nc(fp,xlim=None,ylim=None,disp=True,ax = None,style=False,yoffset=None,title=None,xlabel=r"Wavelength (nm)",line=None,GUI=False,bp=None,subtraction=0):
    if bp:
        join(bp,fp)
